...
public:
    ...
    // void buildHeap()
    // {
    //     vector<vector<signed int>> input = array;
    //     currentSize = input.size();

    //     input.insertionSort();
    // }

    void removeElement(signed int hole)
    {
        array[hole][0] = -1;
        array[hole][1] = -1;
        array[hole][2] = -1;
    ...
    signed int setSkyline(signed int a, signed int b)
    {
        if (a >= b)
        {
            return a;
    ...
    signed int findArrayInLabel(vector<vector<signed int>> &labels, signed int value)
    {
